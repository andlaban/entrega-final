...
function renderizarPelisBuenas() {
	for (const pelicula of peliculasBuenas) {
		...
		poster.className = "card bg-dark col-6 col-sm-4 col-md-3 col-lg-2";
		poster.innerHTML = `
        <div class="card-body fixed-card-body">
            <img src=${pelicula.foto} class="card-img-top rounded" alt=${pelicula.nombre}>
            <h5 class="card-title two-lines">${pelicula.nombre}</h5>
			<div>
            <h5 class="card-text">$ ${pelicula.precio}</h5>
